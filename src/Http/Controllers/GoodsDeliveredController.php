...
class GoodsDeliveredController extends Controller
{
    ...
    {
        if(GoodsDeliveredService::approve($id))
        {
            return [
                'status' => true,
    ...
    {
        return [
            'delete' => route('goods-delivered.delete'),
            'cancel' => route('goods-delivered.cancel'),
        ];
    }

    public function delete(Request $request)
    {
        if (GoodsDeliveredService::destroyMany($request->ids))
        {
            return [
                'status' => true,
                'messages' => [count($request->ids) . ' Goods delivered note(s) deleted.'],
            ];
        }
        else
        {
    ...
    {
        if (GoodsDeliveredService::cancelMany($request->ids))
        {
            return [
                'status' => true,
                'messages' => [count($request->ids) . ' Goods delivered note(s) canceled.'],
            ];
        }
        else
        {
